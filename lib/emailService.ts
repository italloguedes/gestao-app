...
export async function sendEmailConfirmation(agendamento: Agendamento) {
  try {
    ...
    if (!session) {
      console.warn('Sessão não encontrada ao enviar email. Email não será enviado.');
      return; // Não falhar o agendamento por causa do email
    }

    // Chamar API interna com autenticação
    ...
    // Não propagar o erro para não falhar o agendamento
    // throw error;
  }
}
...
export async function sendReminderEmail(agendamento: Agendamento) {
  try {
    const subject = 'Lembrete de Agendamento - Sala Sensorial ALECE';
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
    ...
    if (!session) {
      console.warn('Sessão não encontrada ao enviar email de lembrete.');
      return;
    }
    ...
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(errorData.error || 'Erro ao enviar email de lembrete');
    }
...
export async function sendInstructionsEmail(agendamento: Agendamento) {
  try {
    const subject = 'Instruções para o Atendimento - Sala Sensorial ALECE';
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
    ...
    if (!session) {
      console.warn('Sessão não encontrada ao enviar email de instruções.');
      return;
    }
    ...
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(errorData.error || 'Erro ao enviar email de instruções');
    }

    console.log('Email de instruções enviado com sucesso');
  } catch (error) {
