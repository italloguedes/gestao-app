...
export const generateLotePDF = async ({
    ...
    const drawResumoPage = (startIndex: number, endIndex: number, pageNum: number, totalResumoPages: number, isLastPage: boolean) => {
        // Cabeçalho institucional
        doc.addImage(logoBase64, 'PNG', 15, 10, 25, 25, undefined, 'FAST');
        ...
        if (isLastPage && recebedor) {
            y += 8;
            doc.setFontSize(8); doc.setTextColor(71, 85, 105); doc.setFont('helvetica', 'normal');
            doc.text('Declaro que recebi nesta data as Carteiras de Identidade Nacional (CINs) acima identificadas,', 105, y, { align: 'center' });
            doc.text('estando os documentos em perfeitas condições. Confirmo a veracidade das informações.', 105, y + 4, { align: 'center' });
            ...
            doc.text(recebedor.nome, 105, y + 20, { align: 'center' });
            doc.setFontSize(8); 
            doc.text(`CPF: ${recebedor.cpf}`, 105, y + 24, { align: 'center' });
        }
    ...
    // Calcular quantas páginas de resumo são necessárias
    const maxItemsPerPage = 13;
    const totalResumoPages = Math.ceil(items.length / maxItemsPerPage) || 1;
    ...
    for (let p = 0; p < totalResumoPages; p++) {
        ...
        const isLastPage = (p === totalResumoPages - 1);
        drawResumoPage(startIdx, endIdx, p + 1, totalResumoPages, isLastPage);
    }
