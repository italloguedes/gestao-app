...
export default function ConcluirAtendimento() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (cinError) {
        setMessage({ text: 'Erro ao gerar CIN: ' + cinError.message, type: 'error' });
        return;
      }
      ...
      if (!response.ok) {
        setMessage({ 
          text: 'Atendimento concluído e CIN gerada, mas houve um erro ao enviar o email',
          type: 'error'
        });
        return;
      }
      ...
      setMessage({ 
        text: 'Atendimento concluído e CIN gerada com sucesso! Email enviado.',
        type: 'success'
      });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Concluir Atendimento</h1>
        <p className="text-gray-600 mt-2">Conclua o atendimento e gere a CIN</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-4">
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? <Loading /> : 'Concluir e Gerar CIN'}
            </button>
          </div>
        </form>
      </div>
